...
std::unordered_map<long long, long long> mapValues(const std::vector<long long> &seeds,
                                                 const std::vector<map_value> &values) {
    std::unordered_map<long long, long long> mappedValues;

    for (size_t i = 0; i < seeds.size(); i += 2) {
        long long startSeed = seeds[i];
        long long numSeeds = seeds[i + 1];

        for (long long j = 0; j < numSeeds; ++j) {
            long long seed = startSeed + j;
            bool found = false;
...
vector<long long> getTotalSeeds(vector<long long> seeds_ranges){
  vector<long long> seeds;
  for(long long i = 0LL; i < seeds_ranges.size(); i+=2){
    for(long long j = 0; j < seeds_ranges[i+1]; j++){
...
int main(int argc, char *argv[]) {
  ...
  seeds_ranges = getIntsFromStrings(splitStringBy(tmp[1], " "));
  seeds = getTotalSeeds(seeds_ranges);
  // Get the soil values
  string temp_string = "";
  bool locked = false;
